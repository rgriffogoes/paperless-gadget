...
#
# Update hostname and admin
# Ensure `pass` password storage contains password
#
hostname=paperless-ngx.docker2.home
user=admin
protocol=http

rawpw=$(pass $hostname |head -n1)
pass=$user:$rawpw
...
postdoc(){
	...
	do
		data="@${1}"
		retval=$(curl --retry 99 --retry-all-errors --http1.1 -f -X POST -H "Authorization: Basic $creds" --form "document=$data" -sS $protocol://$hostname/api/documents/post_document/ || exit 1)
		if [[ $? -gt 0 ]]
		then
		...
		then
			...
			do
				...
				taskinfo=$(curl --retry 99 --retry-all-errors --http1.1 -f -X GET -H "Authorization: Basic $creds" -sS $protocol://${hostname}/api/tasks/?task_id=${retval//\"/})
